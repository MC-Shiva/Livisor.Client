...
`Sound/Explosion.mp3`をPrefabのBurst Soundに設定済みです。自動終了・Tキー・公開Fire関数のいずれも、実際に射出するときに1回鳴ります。射出口の数だけ重複再生はしません。最大粒子数に達して射出できないときは鳴りません。

Inspectorの **Burst Volume**（初期値0.7、0〜3）で音量を調整できます。カメラ位置によって聞こえなくならない2D音声で、ライブ停止中にも再生します。音源は初期化時に読み込み、Clearまたはコンポーネント無効化で停止します。別の音にする場合はBurst Soundを差し替えてください。

### 音のタイミング

Inspectorの **Sound Timing Offset** を−2〜＋2秒で調整できます。0で同時、＋1なら射出の1秒後、−1なら音を鳴らしてから1秒後に射出します。マイナス設定ではFireの呼び出しより前に音を出すことはできないため、射出側を遅らせます。自動終了でも同じ動作です。

非スケール時間で待つため、ライブ停止中も動きます。予約中の粒子数も上限に含め、Clearや無効化で待機中の射出・音をキャンセルします。

**Burst Volume** の上限は3です。1を超える設定は音を増幅しますが、音源によって音割れする場合があります。既存の音量0.7は維持しています。
