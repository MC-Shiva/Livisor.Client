...
/// 予約アクション 1 件（<see cref="TimelinePlayback"/>）はサーバー時刻ベースのままで、こちらとは独立。
/// LiveScene（サーバーから受信）と DemoScene（Shared を直接読む）で同じものを使う。
///
...
public sealed class DefaultActionPlayback
{
    ...
    /// 再受信（Live ではトランスポートが届くたびに呼ばれる）で発火済みをやり直さないよう、
    /// 現在の再生位置以前のアクションは飛ばす。
    /// </summary>
